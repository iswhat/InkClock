...
**Web服务器优化**：
- **JavaScript模板字符串修复**：修复web_server.cpp文件中的JavaScript模板字符串问题，将模板字符串（使用反引号）替换为正确的字符串连接方式
- **中文字符支持**：确保web_server.cpp文件中的中文字符能够正常显示，保持中文版内容的完整性
- **插件管理页面修复**：修复在线插件列表加载和显示功能，确保插件信息正确显示
- **错误信息本地化**：确保所有错误信息和提示都使用中文显示，提升用户体验
...
- **JavaScript Template String Fix**: Fixed JavaScript template string issues in web_server.cpp file by replacing template literals (using backticks) with proper string concatenation
- **Chinese Character Support**: Ensured Chinese characters in web_server.cpp file can display correctly, maintaining the integrity of Chinese content
- **Plugin Management Page Fix**: Fixed online plugin list loading and display functionality to ensure plugin information shows correctly
- **Error Message Localization**: Ensured all error messages and prompts are displayed in Chinese to improve user experience
