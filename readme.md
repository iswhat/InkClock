# InkClock - 在线墨水屏时钟万年历系统

[English Version](README_EN.md)

## 项目简介

InkClock是一款基于多种开发板的在线墨水屏时钟万年历系统，支持ESP32、ESP8266、NRF52、STM32、RP2040等多种微控制器平台，具有低功耗、高清晰度、可远程配置等特点。系统集成了完整的万年历功能，支持显示公历、农历、节气、节日等信息，并具备人体感应、光照感应、低功耗优化等特性。同时，系统支持在线插件扩展，可通过配套的管理系统网站进行远程配置和管理。

**硬件和功能的灵活性**：系统采用模块化设计，硬件和功能可根据用户需求随意增减。用户可以根据自己的预算和需求选择不同的开发板、墨水屏尺寸和传感器组合，系统会自动检测并适配硬件配置。功能方面，用户可以通过管理系统网站自由启用或禁用各种功能模块，实现真正的个性化定制。
...
- 📱 **手机端管理**：通过专用APP进行设备配置、状态监控和远程控制
- 💬 **网络文字留言**：支持远程发送文字留言到设备显示
- 🎤 **网络语音留言**：支持远程发送语音留言到设备播放
- 📹 **本地视频留言**：支持本地录制和播放视频留言
