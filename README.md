...
1. **数据库初始化**：
   ...
   - 修改`d:\InkClock\webserver\config\config.php`中的数据库配置

2. **Web服务器配置**：
   - 将`d:\InkClock\webserver`目录部署到Web服务器根目录或子目录
   - 配置Web服务器支持PHP
   - 确保`uploads`目录和`logs`目录具有可写权限

3. **配置文件修改**：
   - 编辑`d:\InkClock\webserver\config\config.php`，修改相关配置项
   - 设置API密钥、文件上传路径、数据库连接信息等

4. **测试部署**：
   - 在浏览器中访问`http://your-server-address/api/status`
   - 若返回JSON格式的状态信息，表示部署成功

##### 新架构说明

服务端已升级为模块化MVC架构，包含以下核心组件：
...
1. **添加新API接口**：
   - 在`config/routes.php`中添加新的路由配置
   - 创建新的控制器类或在现有控制器中添加新方法
   - 实现相应的业务逻辑
   - 遵循RESTful API设计规范

2. **示例**：
   ```php
   // 在config/routes.php中添加新路由
   'GET /api/custom' => 'CustomController@index',
   'POST /api/custom' => 'CustomController@create',
   
   // 创建新的控制器类
   class CustomController extends BaseController {
       public function index($params) {
           // 业务逻辑实现
           $this->response::success('Custom API response');
       }
       
       public function create($params) {
           $data = $this->parseRequestBody();
           // 业务逻辑实现
           $this->response::success('Custom API created', array('data' => $data));
       }
   }
   ```
